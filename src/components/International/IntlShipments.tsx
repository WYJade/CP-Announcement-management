    ...
    <div className="p-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-5">
        <div className="grid grid-cols-3 gap-4 mb-3">
          <div><label className="block text-xs font-medium text-gray-500 mb-1">Shipment Number</label><input type="text" placeholder="Shipment Number" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div>
          <div><label className="block text-xs font-medium text-gray-500 mb-1">House Bill Number</label><input type="text" placeholder="House Bill Number" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div>
          <div><label className="block text-xs font-medium text-gray-500 mb-1">Container Number</label><input type="text" placeholder="Container Number" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" /></div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-gray-200">{['Shipment No','Reference No','Commodity','Vessel Name','Voyage/Flight No','Mode','Container Mode','Shipment Transit Status','Entry Date'].map(h=>(<th key={h} className="text-left py-3 px-3 text-xs font-semibold text-gray-500">{h}</th>))}</tr></thead>
          <tbody>{SAMPLE_DATA.map((row, i) => (<tr key={i} className="border-b border-gray-100 hover:bg-gray-50">
            <td className="py-3 px-3 text-primary-600 font-medium cursor-pointer hover:underline text-xs" onClick={() => navigate(`/international/shipments/${row.shipmentNo}`)}>{row.shipmentNo}</td>
